...
# `from botocore.vendored import requests` | requests library is removed from Botocore Python3.8 and above
import urllib3
...
def send(event, context, response_status, response_data, physical_resource_id,
    ...
    try:
        ...
        logger.error("send(..) failed executing http.request('PUT',...): {}".format(e))
        raise
